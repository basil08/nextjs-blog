...
export function getAllPostIds() {
  ...
  return fileNames.map(fileName => {
    // truncated YYYY-MM-DD- in the name which was reqd for jekyll
    const truncatedFileName = fileName.replace(/^\d{4}-\d{2}-\d{2}-/, '')
    return {
      params: {
        id: truncatedFileName.replace(/\.md$/, '')
      }
    }
  })
}
// returned list is not just an array of strings — it must be an array of objects that look like the comment above.
